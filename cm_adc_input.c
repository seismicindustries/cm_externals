...
static t_class *cm_adc_input_class;

typedef struct _cm_adc_input
{
    t_object x_obj;
    t_outlet *x_out1;
    t_outlet *x_out2;
...
static t_cm_adc_input *cm_adc_input_new(t_floatarg version);
static int cm_adc_input_write_read(t_cm_adc_input *spi, unsigned char *data, int length);
static void cm_adc_input_open(t_cm_adc_input *spi, t_symbol *version);
static int cm_adc_input_close(t_cm_adc_input *spi);
static void cm_adc_input_free(t_cm_adc_input *spi);

/**********************************************************
 * cm_adc_input_open() :function is called by the "open" command
...
static void cm_adc_input_open(t_cm_adc_input *spi, t_symbol *version){
    ...
    #ifdef __arm__
      // we're using CS1 :
      spi->spifd =  open("/dev/spidev1.1", O_RDWR);       // original: /dev/spidev0.1  ->  neu: /dev/spidev1.1
...
/***********************************************************
 * cm_adc_input_close(): Responsible for closing the spidev interface.
 * *********************************************************/
...
static int cm_adc_input_close(t_cm_adc_input *spi){
    ...
    int statusVal = -1;
    if (spi->spifd == -1) {
      pd_error(spi, "cm_adc_input: device not open");
      return(-1);
    }
  #ifdef __arm__
  ...
  #endif
    if(statusVal < 0) {
      pd_error(spi, "cm_adc_input: could not close SPI device");
      exit(1);
    }
...
/***********************************************************
 * cm_adc_input_smooth(): set smoothing
 * *********************************************************/

void cm_adc_input_smooth(t_cm_adc_input *spi, t_floatarg s){
...
/***********************************************************
 * cm_adc_input_deadband(): set deadband
 * *********************************************************/

void cm_adc_input_deadband(t_cm_adc_input *spi, t_floatarg d){
 ...
 * cm_adc_input_MODE_0 (MODE 0) (defined in linux/spi/spidev.h), speed = 1MHz &
 * bitsPerWord=8.
 *
 * *********************************************************************/

static void cm_adc_input_bang(t_cm_adc_input *spi)
{
  #ifdef __arm__
    if (spi->spifd == -1) {
  ...
  for (unsigned int i = 0; i < SMOOTH; i++) {
      ...
      for (a2dChannel = 0; a2dChannel < numChannels; a2dChannel++) {
        ...
        cm_adc_input_write_read(spi, data, 3);
 ...
 * ***********************************************/
static t_cm_adc_input *cm_adc_input_new(t_floatarg version){
    t_cm_adc_input *spi = (t_cm_adc_input *)pd_new(cm_adc_input_class);
...
{
    cm_adc_input_class = class_new(gensym("cm_adc_input"), (t_newmethod)cm_adc_input_new,
        (t_method)cm_adc_input_free, sizeof(t_cm_adc_input), 0, A_DEFSYM, 0);
    class_addmethod(cm_adc_input_class, (t_method)cm_adc_input_open, gensym("open"),
        A_DEFSYM, 0);
    class_addmethod(cm_adc_input_class, (t_method)cm_adc_input_close, gensym("close"),
        0, 0);
    class_addmethod(cm_adc_input_class, (t_method)cm_adc_input_smooth, gensym("smooth"),
        A_DEFFLOAT, 0);
    class_addmethod(cm_adc_input_class, (t_method)cm_adc_input_deadband, gensym("deadband"),
        A_DEFFLOAT, 0);
    class_addbang(cm_adc_input_class, cm_adc_input_bang);
}
